...
///
/// A choice of the selection model is a strong signal that a skill applies.
/// A retrieval rank is only a keyword and vector score. `isSelection` tells a
/// host which of the two it has. `search skill` gives the same plain text for
/// both: the lines of the matches and the instruction to load a skill, and
/// never a body.
